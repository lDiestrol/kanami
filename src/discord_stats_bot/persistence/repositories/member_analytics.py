"""Bounded PostgreSQL reads for member-scoped analytics."""
...
def member_analytics_voice_intervals_statement(
    guild_id: int,
    user_id: int,
    query: AnalyticsQuery,
) -> Select[tuple[int, object, object, str]]:
    """Fetch one member's eligible intervals over the combined report range."""
...
def member_analytics_text_rows_statement(
    guild_id: int,
    user_id: int,
    query: AnalyticsQuery,
) -> Select[tuple[int, object, int]]:
    """Aggregate one member's text activity once per combined-range local date."""
    ...
    return (
        ...
        .where(
            ...
            DailyTextActivity.user_id == user_id,
        ...
        .order_by(DailyTextActivity.activity_date.asc())
    )


def member_analytics_earliest_recorded_statement(
    guild_id: int,
    user_id: int,
) -> Select[tuple[object, object]]:
    """Return earliest source rows for only the requested non-bot member."""
...
class SqlAlchemyMemberAnalyticsRepository:
    """Execute three member-scoped reads on a caller-owned session."""
    ...
    async def list_voice_intervals(
        ...
        rows = (
            await self._session.execute(
                member_analytics_voice_intervals_statement(guild_id, user_id, query)
            )
        ).all()
        return tuple(
            AnalyticsVoiceInterval(
    ...
    async def list_text_rows(
        ...
        rows = (
            await self._session.execute(
                member_analytics_text_rows_statement(guild_id, user_id, query)
            )
        ).all()
        return tuple(
            AnalyticsTextRow(
    ...
    async def get_earliest_recorded(
        ...
        row = (
            await self._session.execute(
                member_analytics_earliest_recorded_statement(guild_id, user_id)
            )
        ).one()
